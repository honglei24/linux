...
usage() {
  ...
  -T  Is there a salt-syndic in the environment

EOT
}   # ----------  end of function usage  ----------


while getopts ":hvnDc:Gg:k:MSNXCPFUKIA:i:Lp:dH:ZTm:" opt
do
  case "${opt}" in
...
if [ "$_INSTALL_MASTER" -eq $BS_TRUE ] && [ "$_SALT_MASTER_ADDRESS" != "null" ]; then
    echoerror "Don't pass a master address (-A) if master is going to be bootstrapped."
    exit 1
fi
...
__config_salt_etc_salt(){
	# Configuration Start
	echodebug "Configuration salt(master/minion/syndic)!"
	
	_CPU_CORE_NUM=`grep process /proc/cpuinfo | wc -l` || return 1
    if [ "$_INSTALL_MINION" -eq $BS_TRUE ]; then
	    [ -f ${_SALT_ETC_DIR}/minion ] && \cp ${_SALT_ETC_DIR}/minion ${_SALT_ETC_DIR}/minion.`date +"%Y%m%d%H%M%S"` 
        cat << EOF > ${_SALT_ETC_DIR}/minion || return 1
...
EOF
    ...
    if [ "$_INSTALL_MASTER" -eq $BS_TRUE ];then
	    [ -f ${_SALT_ETC_DIR}/master ] && \cp ${_SALT_ETC_DIR}/master ${_SALT_ETC_DIR}/master.`date +"%Y%m%d%H%M%S"` 
		id saltadmin >/dev/null 2>&1 || useradd saltadmin || return 1
        cat << EOF > ${_SALT_ETC_DIR}/master
worker_threads: ${_CPU_CORE_NUM}
file_roots:
  base:
    - /srv/salt
file_ignore_regex:
...
external_auth:
  pam:
    saltadmin:
      - '*'
EOF
       [ ${_IS_TOP_MASTER} -eq $BS_TRUE ] && echo "order_masters: true" >>${_SALT_ETC_DIR}/master 
    fi
    if [ "$_INSTALL_SYNDIC" -eq $BS_TRUE ];then
	    [ -f ${_SALT_ETC_DIR}/syndic ] && \cp ${_SALT_ETC_DIR}/syndic ${_SALT_ETC_DIR}/syndic.`date +"%Y%m%d%H%M%S"` 
		id saltadmin >/dev/null 2>&1 || useradd saltadmin || return 1
        cat << EOF > ${_SALT_ETC_DIR}/syndic
pillar_opts: false
file_roots:
  base:
    - /srv/salt
...
external_auth:
  pam:
    saltadmin:
      - '*'
syndic_master: ${_SALT_MASTER_ADDRESS}
zmq_filtering: True
EOF
    fi
...
install_ubuntu_stable() {
    ...
    # shellcheck disable=SC2086
    __apt_get_install_noinput ${__PACKAGES} || return 1
	__config_salt_etc_salt || return 1
    return 0
}
...
install_centos_stable_deps() {
    echodebug "Running install_centos_stable_deps!"
    ...
    if [ "$_ENABLE_EXTERNAL_ZMQ_REPOS" -eq $BS_TRUE ] && [ "$DISTRO_MAJOR_VERSION" -gt 5 ]; then
        yum -y install python-hashlib > /dev/null 2>&1 || return 1
        __install_saltstack_copr_zeromq_repository || return 1
    fi
    ...
    if [ -f "${__SALT_GIT_CHECKOUT_DIR}/requirements/base.txt" ]; then
        ...
        __REQUIRED_TORNADO="$(grep tornado "${__SALT_GIT_CHECKOUT_DIR}/requirements/base.txt")"
        if [ "${__REQUIRED_TORNADO}" != "" ]; then
            yum install -y python-tornado > /dev/null 2>&1
        fi
    fi

    if [ "$_UPGRADE_SYS" -eq $BS_TRUE ]; then
        yum -y update > /dev/null 2>&1 || return 1
    fi
    ...
    if [ "$DISTRO_NAME_L" = "oracle_linux" ]; then
        ...
        for package in ${__PACKAGES}; do
            ...
            yum -y install ${package} > /dev/null 2>&1 || yum -y install ${package} > /dev/null 2>&1 || return 1
        done
    else
        # shellcheck disable=SC2086
        yum -y install ${__PACKAGES} --enablerepo=${_EPEL_REPO} > /dev/null 2>&1 || return 1
    fi
    ...
    if [ "${_EXTRA_PACKAGES}" != "" ]; then
        ...
        if [ "$DISTRO_NAME_L" = "oracle_linux" ]; then
            ...
            for package in ${_EXTRA_PACKAGES}; do
                # shellcheck disable=SC2086
                yum -y install ${package} > /dev/null 2>&1 || yum -y install ${package} --enablerepo=${_EPEL_REPO} > /dev/null 2>&1 || return 1
            done
        else
            # shellcheck disable=SC2086
            yum install -y ${_EXTRA_PACKAGES} --enablerepo=${_EPEL_REPO} > /dev/null 2>&1 || return 1
        fi
    fi
...
install_centos_stable() {
    echodebug "Running install_centos_stable!"
    __PACKAGES=""
    if [ "$_INSTALL_MINION" -eq $BS_TRUE ]; then
        __PACKAGES="${__PACKAGES} salt-minion"
    fi
...
external_auth:
  pam:
    saltadmin:
      - '*'
EOF
       [ ${_IS_TOP_MASTER} -eq $BS_TRUE ] && echo "order_masters: true" >>${_SALT_ETC_DIR}/master || return 1
    fi
    if [ "$_INSTALL_SYNDIC" -eq $BS_TRUE ];then
	    [ -f ${_SALT_ETC_DIR}/syndic ] && \cp ${_SALT_ETC_DIR}/syndic ${_SALT_ETC_DIR}/syndic.`date +"%Y%m%d%H%M%S"` 
		id saltadmin >/dev/null 2>&1 || useradd saltadmin || return 1
...
install_centos_stable_post() {
    echodebug "Running install_centos_stable_post!"
    for fname in minion master syndic api; do
        # Skip if not meant to be installed
        # Skip salt-api since the service should be opt-in and not necessarily started on boot
        [ $fname = "api" ] && continue
...
install_centos_git_deps() {
    ...
    if [ "$(which git)" = "" ]; then
        ...
        if [ "$DISTRO_NAME_L" = "oracle_linux" ]; then
            ...
            yum install -y git > /dev/null 2>&1 || yum install -y git --enablerepo=${_EPEL_REPO} > /dev/null 2>&1 || return 1
        else
            yum install -y git --enablerepo=${_EPEL_REPO} > /dev/null 2>&1 || return 1
        fi
    fi

    if [ "$DISTRO_MAJOR_VERSION" -gt 6 ]; then
        if [ "$DISTRO_NAME_L" != "oracle_linux" ]; then
            yum install -y systemd-python > /dev/null 2>&1 || yum install -y systemd-python --enablerepo=${_EPEL_REPO} > /dev/null 2>&1 || return 1
        else
            yum install -y systemd-python --enablerepo=${_EPEL_REPO} > /dev/null 2>&1 || return 1
        fi
    fi
...
install_centos_check_services() {
    echodebug "Running install_centos_check_services!"
    for fname in minion master syndic api; do
        # Skip salt-api since the service should be opt-in and not necessarily started on boot
        [ $fname = "api" ] && continue
...
install_opensuse_stable_deps() {
    ...
    if [ $? -eq 1 ]; then
		...
		__zypper addrepo --refresh \
            "http://download.opensuse.org/repositories/devel:/languages:/python/SLE_11_SP3/devel:languages:python.repo" || return 1
    fi
...
install_suse_11_stable_deps() {
    ...
    if [ $? -eq 1 ]; then
        ...
        __zypper addrepo --refresh \
            "http://download.opensuse.org/repositories/devel:/languages:/python/${DISTRO_REPO}/devel:languages:python.repo" || \
		__zypper addrepo --refresh \
            "http://download.opensuse.org/repositories/devel:/languages:/python/SLE_11_SP3/devel:languages:python.repo" || \
			return 1
    fi
...
# Configure Salt
echodebug "Configure Salt"
echodebug "TEMP_CONFIG_DIR=$_TEMP_CONFIG_DIR"
echodebug "CONFIG_SALT_FUNC=$CONFIG_SALT_FUNC"
